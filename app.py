...
st.sidebar.caption("Pick what applies. US²DF will take the maximum (binding constraint) across what you select.")

use_precision = st.sidebar.checkbox(
    "Precision component",
    value=True,
    help=(
        "Use this if your goal includes estimating a proportion/mean within a chosen margin of error. "
        "It applies the Adam (2020) finite-population precision formula."
    )
)
use_power = st.sidebar.checkbox(
    "Power component",
    value=True,
    help=(
        "Use this if your goal includes detecting differences/associations. "
        "US²DF uses empirically validated benchmark sample sizes per group: "
        "Small=400, Medium=100, Large=50, then scales by number of groups."
    )
)
use_model = st.sidebar.checkbox(
    ...
    value=False,
    help=(
        "Use this if you will run an explicit model (regression, logistic, SEM/CFA). "
        "It applies planning heuristics like Green (1991), EPV for logistic, or SEM n-per-parameter."
    )
)
...
st.sidebar.caption("Only used if Precision is selected.")
...
e = float(
    st.sidebar.number_input(
        ...
        key=e_key,
        help="Defaults: 0.05 for categorical, 0.03 for continuous."
    )
)
...
# ============================================================
# Power settings (benchmarks only)
# ============================================================
st.sidebar.markdown("### Step 4, Power settings")
st.sidebar.caption("Only used if Power is selected. Benchmarks are PER GROUP and drive the sample size.")
...
effect_size = st.sidebar.radio(
    ...
    help="US²DF benchmark per-group sample sizes: Small=400, Medium=100, Large=50."
)
...
design_type = st.sidebar.selectbox(
    ...
    index=1,
    disabled=not use_power,
    help="Benchmarks are per group. Total n = per-group benchmark × number of groups."
)
...
# Optional reporting defaults (do not affect benchmark n)
with st.sidebar.expander("Optional, reporting defaults (do not change benchmark n)", expanded=False):
    st.caption(
        "These values are shown in the Methods text for reporting conventions. "
        "US²DF benchmark power does not recompute n from α and power here."
    )
...
# If the expander wasn’t opened, keep defaults for Methods text
...
st.sidebar.caption("Only used if Model is selected.")
...
if use_power:
    ...
    if design_type == "Single group (one sample)":
        ...
        power_note = f"US²DF benchmark: {effect_size}={n_power_per_group} (single group). Reporting defaults: α={alpha:.3f}, power={target_power:.2f}."
    elif design_type == "Two independent groups":
        n_power = n_power_per_group * 2
        power_note = f"US²DF benchmark: {effect_size}={n_power_per_group} per group × 2 groups = {n_power} total. Reporting defaults: α={alpha:.3f}, power={target_power:.2f}."
    else:
        n_power = n_power_per_group * groups_k
        power_note = f"US²DF benchmark: {effect_size}={n_power_per_group} per group × k={groups_k} = {n_power} total. Reporting defaults: α={alpha:.3f}, power={target_power:.2f}."
...
st.write(
    "US²DF combines the selected components, applies the max-rule (binding constraint), "
    "then adjusts for field conditions (DEFF, HVIF, nonresponse)."
)

c1, c2, c3 = st.columns(3)
c1.metric("Precision-based sample size", f"{n_precision:,}" if n_precision is not None else "—")
c2.metric("Power-based sample size", f"{n_power:,}" if n_power is not None else "—")
c3.metric("Model-based sample size", f"{n_model:,}" if n_model is not None else "—")
